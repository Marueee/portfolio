...
// Seeded fallback data so it renders offline / rate-limited
...
export default function GitHubStats() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await fetch(
          "https://api.github.com/users/Marueee",
    ...
    const fetchContributions = async () => {
      try {
        const res = await fetch(
          "https://github-contributions-api.jogruber.de/v4/Marueee?y=last",
    ...
    <section className="section-gap max-w-6xl mx-auto relative z-10">
      ...
      >
        <div className="flex items-start gap-4 mb-8">
          <span className="font-mono text-[15px] tracking-[0.1em] text-moon-mist/80 uppercase pt-1">
            Open Source
          </span>
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-12">
            ...
            <div className="glass-card-light p-6 text-center">
              {displayUser.avatar_url !== "https://avatars.githubusercontent.com/u/0?v=4" ? (
                <img
                  ...
                  className="w-20 h-20 rounded-full mx-auto mb-4 ring-2 ring-glass-edge"
                />
              ) : (
              ...
              <h3 className="text-lg font-medium text-ice-highlight mb-1">
                {displayUser.name || displayUser.login}
              </h3>
              <p className="text-sm text-fog font-mono mb-4">
                @{displayUser.login}
              </p>

              <div className="space-y-3 text-center">
                <div className="flex items-center justify-center gap-2 text-sm text-frost-glow/80">
                  <Code className="w-4 h-4 text-violet/60" />
                  <span>{displayUser.public_repos} repos</span>
                </div>
                <div className="flex items-center justify-center gap-2 text-sm text-frost-glow/80">
                  <Users className="w-4 h-4 text-blueprint/60" />
                  <span>{displayUser.followers} followers</span>
                </div>
              </div>

              <a
                href={`https://github.com/${displayUser.login}`}
                target="_blank"
                rel="noopener noreferrer"
                className="btn-primary mt-6 w-full justify-center text-sm py-3"
              >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm font-mono text-moon-mist/80 uppercase tracking-wider">
            ...
            <div className="flex items-end gap-[3px] h-12">
              {displayContributions.slice(0, 128).map((day, i) => (
                <div
                  key={i}
                  className={`w-[9px] md:w-[11px] h-1 rounded-sm ${levelColors[day.level] || "bg-steel/30"} transition-all duration-200 hover:opacity-80`}
            ...
            <div className="flex items-center gap-4 mt-4 text-xs text-fog font-mono">
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 rounded-sm bg-steel/30" />
                <span>Less</span>
              </div>
              <div className="flex items-center gap-1">
              ...
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 rounded-sm bg-violet-400/70" />
                <span>4+</span>
              </div>
            </div>

            {displayTotal.lastYear > 0 && (
              <p className="text-sm text-frost-glow/60 mt-3 font-mono">
                {displayTotal.lastYear} contributions total in the last year
              </p>
            )}
          </div>
        </div>
